...
const discrete = { id: "d", name: "d", kind: "discrete", ramGB: 32, vramGB: 24, bwGB: 800, bwCpuGB: 55, ppRate: 3000 };
...
describe("budget: architecture decides the memory model", () => {
  ...
  it("unified's slow pool stays fast; discrete's does not", () => {
    ...
    ok(d.bwSlow / d.bwFast < 0.2, "discrete slow pool is across PCIe");
  });
});
...
describe("the central claim: spilling differs by architecture", () => {
  ...
  it("discrete falls off a cliff for the same spill", () => {
    ok(spillPenalty(discrete, { ...moe30b, weightsGB: 26 }, 32768) < .3,
       "PCIe spill should be catastrophic");
  });
  it("the gap between them is large, not marginal", () => {
    const u = spillPenalty(unified, { ...dense9b, weightsGB: 11.5 }, 32768);
    const d = spillPenalty(discrete, { ...moe30b, weightsGB: 26 }, 32768);
    ok(u / d > 2, `unified should retain far more speed than discrete (ratio ${(u / d).toFixed(1)})`);
  });
});
